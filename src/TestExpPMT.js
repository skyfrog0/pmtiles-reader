...
const {toHexString} = require('./HexUtil');
...
(async () => {
    const filePath = path.resolve("C:\\Users\\skyfr\\Desktop\\TEMP\\pmt-exp\\s3857gwt.pmtiles");
    ...
    console.log('\n=== PMT Leaf Directories (show the 1st only) ===');
    const firstEntry = pmtDicts[0];
    if (0 == firstEntry.runLength) {  // 第一个叶目录
        const leafEntries = await PMT.cache.getDirectory(filesrc, pmtHeader.leafDirectoryOffset + firstEntry.offset, firstEntry.length, pmtHeader);
        const top15 = leafEntries.slice(0, 15);
        top15.map(item => { item.zxy = pmtjs.tileIdToZxy(item.tileId)});

        console.log(util.inspect(top15, {depth: null, colors: true, compact: true}));
    }

    // 获取瓦片
    console.log('\n=== PMT 获取瓦片 ===');
    console.log("\n正在获取瓦片 z=8, x=104, y=55...");
    const tileResult = await PMT.getZxy(15, 13374, 7005);
    if (!tileResult) {
        console.log("瓦片不存在");
        return null;
    }
